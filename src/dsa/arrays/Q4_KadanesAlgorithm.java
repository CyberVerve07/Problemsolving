 ...
 *  🏢 Asked In: Amazon, Microsoft, Google, Adobe, Paytm, Flipkart
 *  ⭐ Difficulty: Medium
 ...
 *  ❌ O(n³) → way too slow for interviews
 *
 * ============================================================
 ...
 *  → A negative sum will always REDUCE the next element.
 ...
 *    i=1: num= 1 → cur=-2+1=-1 < 0? No → cur=-1, max=-1... wait:
 *         Actually cur = max(1, -2+1) = 1 → cur=1, max=1
 *    i=2: num=-3 → cur=1-3=-2, max=1
 *    i=3: num= 4 → cur=-2+4=2 > 0, OR start fresh: 4 > 2 → cur=4, max=4
 *    i=4: num=-1 → cur=4-1=3, max=4
 *    i=5: num= 2 → cur=3+2=5, max=5
 *    i=6: num= 1 → cur=5+1=6, max=6 
 *    i=7: num=-5 → cur=6-5=1, max=6
 *    i=8: num= 4 → cur=1+4=5, max=6
 *    Final maxSum = 6 
 *
 *  SIMPLE RULE: if (currentSum < 0) → reset currentSum = 0
 *
 * ============================================================
 */
public class Q4_KadanesAlgorithm {
    ...
    public static void main(String[] args) {
         ...
         * 3. Time: O(n), Space: O(1) → Optimal!
         * 4. Named "Kadane's Algorithm" → Say the name in interviews!
         * 5. FOLLOW-UP: "Print the actual subarray" → track start/end indices
